    ...
    window.addEventListener("message", function(event){
        ...
        switch(event.data.action){
            case 'opensystem':
                ...
                $.post("https://vrp_bank/getGraphic")
        ...
        document.onkeyup = function(data){
            if(data.which == 27) {
                $.post("https://vrp_bank/fecharbanco")
                closeModal()
                inMultas = false
...
function SendMulta(){
    ...
    if(valormulta > 0 ) {
        ...
        closeModalPix()
        $.post("https://vrp_bank/multas",JSON.stringify({
            type: 'apply',
            id: passaporte,
            value: valormulta,
            reason: reason,
...
function DepositPix(){
    ...
    if(pixvalue > 0 ) {
        ...
        closeModalPix()
        $.post("https://vrp_bank/pix",JSON.stringify({
            type: 'send',
            key: pixchave,
            value: pixvalue
        }),(data) => {
...
function spawnMoney() {
    ...
    if(money > 0){
        closeModal()
        $.post("https://vrp_bank/money",JSON.stringify({
            type: 'drop',
            param: money,
        }),(data) => {
            if(data.retorno == 'sucesso'){
...
function clear_trans() {
    ...
    $.post("https://vrp_bank/clearTrans",JSON.stringify({}),(data) => {
        if(data.retorno == 'sucesso'){
            $.post("https://vrp_bank/getGraphic")
        }
    });
...
function ConsultarMultas(){
    $('.content-traffic').empty()
    $.post('https://vrp_bank/multas',JSON.stringify({type: 'consultar'}),(data) => {
